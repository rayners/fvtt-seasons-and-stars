...
describe('ExternalCalendarRegistry', () => {
  ...
  describe('Protocol Handler Registration', () => {
    ...
    it('should throw error when registering duplicate protocol', () => {
      registry.registerHandler(mockHandler);
      expect(() => registry.registerHandler(mockHandler)).toThrow('Protocol mock already registered');
    });
  });
  ...
  describe('External Calendar Loading', () => {
    ...
    it('should throw error for invalid external calendar ID format', () => {
      expect(() => registry.parseExternalCalendarId('invalid-format')).toThrow('Invalid external calendar ID format');
    });
    ...
    it('should throw error for unsupported protocol', async () => {
      const result: LoadCalendarResult = await registry.loadExternalCalendar('unsupported:location');
  ...
  describe('Calendar Caching', () => {
    ...
    it('should cache loaded calendars', async () => {
      ...
      const result1: LoadCalendarResult = await registry.loadExternalCalendar('mock:test://cacheable');
      expect(result1.fromCache).toBe(false);
      
      // Second load - should be from cache
      const result2: LoadCalendarResult = await registry.loadExternalCalendar('mock:test://cacheable');
    ...
    it('should force refresh when requested', async () => {
      ...
      const result = await registry.loadExternalCalendar('mock:test://forceable', { forceRefresh: true });
      expect(result.fromCache).toBe(false);
    });
  ...
  describe('External Calendar Sources Management', () => {
    it('should add external calendar source', () => {
      ...
      const addedSource = sources.find(s => s.protocol === 'https' && s.location === 'example.com/calendar.json');
    ...
    it('should remove external calendar source', () => {
      ...
      expect(sourcesAfterAdd.some(s => s.protocol === 'https' && s.location === 'example.com/removable.json')).toBe(true);
      
      registry.removeExternalSource('https:example.com/removable.json');
      const sourcesAfterRemove = registry.getExternalSources();
      expect(sourcesAfterRemove.some(s => s.protocol === 'https' && s.location === 'example.com/removable.json')).toBe(false);
    });
    ...
    it('should update external source configuration', () => {
      ...
      const sources = registry.getExternalSources();
      const updated = sources.find((s: ExternalCalendarSource) => s.location === 'example.com/updatable.json');
