...
describe('Calendar Note Permission Filtering', () => {
  ...
  describe('Journal Permission Mirroring Verification', () => {
    it('should use the same permission logic as Foundry journal entries', () => {
      ...
      const testNote = {
        isVisibleToUser: (user: any) => {
          ...
          const userLevel =
            ownership[user.id] || ownership.default || mockCONST.DOCUMENT_OWNERSHIP_LEVELS.NONE;
    ...
    it('should properly handle all Foundry ownership levels', () => {
      ...
      expect(
        createNoteWithOwnership(mockCONST.DOCUMENT_OWNERSHIP_LEVELS.NONE).isVisibleToUser(
          playerUser
        )
      ).toBe(false); // NONE level should not be visible

      expect(
        createNoteWithOwnership(mockCONST.DOCUMENT_OWNERSHIP_LEVELS.LIMITED).isVisibleToUser(
          playerUser
        )
      ).toBe(false); // LIMITED level should not be visible (< OBSERVER)

      expect(
        createNoteWithOwnership(mockCONST.DOCUMENT_OWNERSHIP_LEVELS.OBSERVER).isVisibleToUser(
          playerUser
        )
      ).toBe(true); // OBSERVER level should be visible

      expect(
        createNoteWithOwnership(mockCONST.DOCUMENT_OWNERSHIP_LEVELS.OWNER).isVisibleToUser(
          playerUser
        )
