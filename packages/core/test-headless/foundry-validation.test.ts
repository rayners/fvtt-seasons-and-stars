...
test.describe('Foundry Login Flow Discovery', () => {
  test('should explore Foundry homepage and document actual selectors', async ({ page }) => {
    ...
    try {
      ...
      const adminPasswordField = page.locator('input[name="adminKey"], input[name="password"], input[type="password"]');
      ...
      const postAuthContent = await page.evaluate(() => {
        const worldElements = Array.from(document.querySelectorAll('[data-world-id], .world, .world-entry')).map(el => ({
  ...
  test('should attempt to join the first available world', async ({ page }) => {
    ...
    try {
      ...
      // Join the world
      const joinButton = page.locator('button[name="join"], button:has-text("Join"), button:has-text("Launch")');
