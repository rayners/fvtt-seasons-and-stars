...
foundryTest.describe('Foundry Connectivity Tests', () => {
  foundryTest('should connect to Foundry instance', async ({ foundryClient }) => {
    ...
    await expect(joinButton.or(page.locator('[data-world-id]')).first()).toBeVisible({
      timeout: 10000,
    });
  });
  ...
  foundryTest(
    'should login as Player to D&D5e world',
    async ({ playerClient, dnd5eWorld, playerUser }) => {
  ...
  foundryTest(
    'should verify Seasons & Stars module is active',
    async ({ foundryClient, pf2eWorld, gmUser }) => {
  ...
  foundryTest('should verify different calendar systems per world', async ({ browser, gmUser }) => {
    ...
    try {
      ...
      const pf2eWorld = (await import('./FoundryTestClient')).DEFAULT_CONFIG.worlds.find(
        w => w.system === 'pf2e'
      )!;
      ...
      const dnd5eWorld = (await import('./FoundryTestClient')).DEFAULT_CONFIG.worlds.find(
        w => w.system === 'dnd5e'
      )!;
  ...
  foundryTest(
    'should test clean slate reset functionality',
    async ({ gmClient, pf2eWorld, gmUser }) => {
      await gmClient.connectToWorld(pf2eWorld.id, gmUser.id);
  ...
  foundryTest(
    'should verify widget elements are present',
    async ({ foundryClient, pf2eWorld, playerUser }) => {
